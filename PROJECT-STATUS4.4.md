...
**Next Action:** Start a fresh chat for Stage 5 (Knowledge Index Page). Paste this file into the first message. Claude will begin by confirming the two carry-over decisions already made at the end of Stage 4: (a) the Insights index is a chronological feed with search (no category filters, decided in Stage 4), and (b) the marquee ticker will deploy on the Insights index (confirmed in Stage 4). Stage 5 itself will produce `/en/knowledge.html` — the "library view" that lists every published article, plus the small search/filter UI needed to make a category-less feed navigable as it grows. **Live preview is now working** after debugging session uncovered and fixed a class-name mismatch between Stage 2's `motion.css` and Stage 3's `animations.js` — see Parked Items for full details.
...
- **Live preview debugging (NEW, parked mid-Stage 4):** User attempted to preview the site via GitHub Pages at `https://phuwanutkhodop.github.io/accountingsite.github.io/`. Four issues surfaced and were resolved: (1) repo named `accountingsite.github.io` under user `phuwanutkhodop` meant GitHub published as a project site rather than a user-site — URL became `phuwanutkhodop.github.io/accountingsite.github.io/` with repo-name in the path (not a bug, just the URL pattern) (2) root `index.html` was an accidental duplicate of `/en/index.html` with `../theme/` paths that don't work at the root → replaced with a proper redirect file (3) `/en/index.html` contained a nested-comment anti-pattern on line 18 (`<!-- EDIT: -->` literal text inside an outer banner comment) that broke HTML comment parsing and leaked banner text onto the page → patched (4) **THE REAL ROOT-CAUSE BUG:** class-name mismatch between `motion.css` (looking for `.in-view`) and `animations.js` (adding `.is-revealed`). This caused every `data-animate` element to stay at opacity 0 forever — only Section 1 of the homepage was visible, because it uses pure-CSS on-load animations that don't depend on the scroll-reveal system. Sections 2–7 all used `data-animate` and were therefore invisible. Fixed by changing `motion.css` to match the JS's class name. This bug originated from the fact that Stages 2 and 3 were built in separate chat sessions and the class name drifted between them. Resolved.
- **Template anti-pattern to remember (NEW, lesson from Stage 4 preview debug):** Never write comments that *describe* HTML comment syntax using literal `<!--` or `-->` inside another comment. The HTML parser treats any `-->` as a close, even inside what looks like a larger comment block. When describing comment markers in documentation comments, use words ("EDIT markers") or alternative phrasing instead of showing the syntax. Apply this rule to Stages 5, 6, 7, 8, 9 and to any future edits to Stage 2/3/4 files.
- **Cross-session consistency risk (NEW, lesson from Stage 4 preview debug):** Building each stage in a fresh chat is good for context management, but creates drift risk when files from different stages need to interact (CSS class names, JS selectors, data attributes). Mitigation for future stages: at the start of any stage that touches a file already built, re-read the existing file first rather than assuming its internal conventions from memory. Especially important in Stages 5 (knowledge.html reads from article files), 6 (SEO layer reads from every page), 7 (docs reference everything).
...
| 24 Apr 2026 | Post-Stage-4 preview attempt on GitHub Pages. Four issues investigated: project-site URL structure (not a bug, expected), duplicate root index.html (patched to redirect), nested-comment anti-pattern in `/en/index.html` (patched), and — the real root cause — class-name mismatch between `motion.css` (`.in-view`) and `animations.js` (`.is-revealed`) causing Sections 2–7 to stay invisible forever. Fixed by updating `motion.css`. All four issues resolved. Two lessons captured: no literal comment syntax inside comments; always re-read existing files when a new stage touches them rather than trusting memory for internal conventions. |
